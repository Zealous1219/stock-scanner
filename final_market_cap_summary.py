...
def generate_correct_market_cap_summary():
    """生成正确的市值汇总报告"""
    ...
    # 将市值从万元转换为亿元（除以10000）
    df_with_market_cap['market_cap_billion'] = df_with_market_cap['market_cap'] / 10000
    ...
    print("-" * 100)
    print(f"{'排名':<4} {'代码':<12} {'名称':<20} {'市值(亿元)':<12} {'市盈率':<10} {'行业':<25}")
    print("-" * 100)
    ...
    for i, (idx, row) in enumerate(df_with_market_cap_sorted.iterrows(), 1):
        ...
        print(f"{i:<4} {code:<12} {name[:18]:<20} {market_cap:<12} {pe_ratio:<10} {industry[:23]:<25}")
    ...
    # 保存修正后的文件
    output_file = "output/black_horse_candidates_with_market_cap_final_2026-04-15.csv"

    # 创建修正后的DataFrame
    df_final = df_with_market_cap_sorted.copy()
    df_final['market_cap_亿元'] = df_final['market_cap_billion']
    df_final['total_share_亿股'] = df_final['total_share'] / 10000

    # 选择需要的列
    columns_to_keep = ['code', 'name', 'market_cap_亿元', 'pe_ratio', 'pb_ratio', 'industry',
                      'total_share_亿股', 'close_price', 'net_profit']

    # 只保留存在的列
    existing_columns = [col for col in columns_to_keep if col in df_final.columns]
    ...
    return df_with_market_cap_sorted

if __name__ == "__main__":
    generate_correct_market_cap_summary()